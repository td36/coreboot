...
static VOID *FindCbTag (struct lb_header *cbtable, UINT32 Tag)
{
...
static void build_gfx_info_hob (struct lb_header *cbtable)
{
  ...
  lb_fb = FindCbTag (cbtable, LB_TAG_FRAMEBUFFER);
  if (!lb_fb) {
...
static void build_serial_hob(struct lb_header *cbtable)
{
  ...
  lbSerial = FindCbTag (cbtable, LB_TAG_SERIAL);
  if (!lbSerial) {
...
static void build_memory_hobs (struct lb_header *cbtable)
{
  ...
  rec = (struct lb_memory *)FindCbTag (cbtable, LB_TAG_MEMORY);
  if (!rec) {
...
{
  EFI_PHYSICAL_ADDRESS            extra_data_base;
  ...
  for (seg = cbfssegs;; ++seg) {
    if (read_be32(&(seg + 1)->type) == PAYLOAD_SEGMENT_ENTRY) {
      printk(BIOS_DEBUG, "report uefi_fv addr\n");
      cbfs_decode_payload_segment(&segment, seg);
      extra_data_base = segment.load_addr;
      extra_data_size = segment.mem_len;
      break;
    }
  }

  length = sizeof (UNIVERSAL_PAYLOAD_EXTRA_DATA) + sizeof (UNIVERSAL_PAYLOAD_EXTRA_DATA_ENTRY);
  extra_data_hob = BuildGuidHob (&extra_data_guid, length);
  if (!extra_data_hob) {
    printk(BIOS_WARNING, "Fail to extra_data hob");
    return;
  }

  extra_data_hob->header.revision = 1;
  extra_data_hob->header.length   = length;
  extra_data_hob->count           = 1;
  extra_data_hob->entry[0].base   = extra_data_base;
  extra_data_hob->entry[0].size   = extra_data_size;
  strncpy(extra_data_hob->entry[0].identifier, "uefi_fv", sizeof (extra_data_hob->entry[0].identifier));
  return;
}
